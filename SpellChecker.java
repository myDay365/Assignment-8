...
public class SpellChecker {
	 ...
	 * 
	 * @param dictionary_file - the File that contains Strings used to build the
	 *                        dictionary
	 */
	...
	public List<String> spellCheck(File documentFile) {
		...
		for (String word : wordsToCheck) {
			boolean spelledRight = dictionary.contains(word);
			if (!spelledRight) {
				misspelled.add(word);
			}
		}
	 ...
	 * symbols, digits, and spaces are ignored.) Also note that all words are
	 * lower-cased
	 * 
	...
	private List<String> readFromFile(File file) {
		...
		// Put this back if you need to for debugging purposes
